...
records.forEach((row: any, index: number) => {
  try {
    const parsed = PortSchema.parse({
      ...
      passenger_volume: row.passenger_volume ? parseInt(row.passenger_volume) : undefined,
      volume_year: row.volume_year ? parseInt(row.volume_year) : undefined,
...
// Compute neighbors (inline, threshold 200 km for MVP – adjust later)
